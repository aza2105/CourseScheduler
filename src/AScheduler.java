...
//	private static boolean random = true;
	...
	PriorityQueue<ASemester> frontier = new PriorityQueue<ASemester>();

	// an active semester object
	private ASemester activeSemester;
...
//	private PriorityQueue<Semester> addToSorted = new PriorityQueue<Semester>();
	...
	{ 
		...
		for ( String s : Preferences.prefs.getCoursesTaken() ) {
			System.out.println( " Adding "+s+"to root courses");
	...
	public LinkedList<ASemester> uniformCostSearch()
	{
		LinkedList<ASemester> optimalSemesterList = new LinkedList<ASemester>();
		...
		ASemester sem = new ASemester(0, -1, 0, 2014, null, null, rootInheritedCourses, 0.0, 0.0);

		// set the root to active
		activeSemester = sem;

		// maintain explored?
		Set<ASemester> explored = new HashSet<ASemester>();
		...
		ArrayList<ASemester> childSem = new ArrayList<ASemester>();
		...
		while (true)
		{
			...
			if (frontier.isEmpty())
			{
				...
				if (activeSemester.getDepth() <= numCourses )		{
					...
					ArrayList<ASemester>temp = new ArrayList<ASemester>();
					
					// add new children to the frontier
					for ( int i=0; i<childSem.size(); i++ ) {
						frontier.offer( childSem.get(i) );						
					}
	...
	private boolean succeedsGoalTest(ASemester semester)
	{
	...
	public static void main(String[] args) {
		...
		AScheduler s = new AScheduler();// req );
		
		LinkedList<ASemester> test = new LinkedList<ASemester>();
