 ...
 * Date: 7/3/14
 * 
 * Title: ASemester.java
 * 
 *
 * ASemester here refers to an object containing a proposed semester schedule
 * For instance, a proposed schedule for Fall 2014 would be 
 * considered an instance of ASemester. A different proposed schedule
 * for Fall 2014 would be a different ASemester instance.
 * 
 ...
 * generated, and their path costs, to AScheduler, which maintains a 
 * priority queue of the semester object nodes to expand next.
 * 
 */
import java.util.*;

public class ASemester implements Comparable<ASemester> {
	/*
	 * CONSTANTS
	 */
	...
	private ArrayList<ASemester> children;
	...
	private ASemester completedSemester;

	// previous semester aka parent in the tree
	private ASemester parentSemester;
	 ...
	 * CONSTRUCTORS
	 */
	public ASemester(int depth, int term, int sID, int sYear, Set<Section> sect,
			ASemester pSemester, Set<Course> iCourses, double inheritedPathCost, 
			double hX )

	{
		...
		this.hvalue = hX;
		this.children = new ArrayList<ASemester>();

		this.id = nodeID;
		nodeID++;

		if (AScheduler.semesterBreakpoints.contains(Integer.valueOf(depth + 1))) {
	...
	public ASemester getParentSemester() {
		return parentSemester;
	}

	public void setParentSemester(ASemester p) {
		parentSemester = p;
	}
	...
	public LinkedList<ASemester> getFinal() {

		LinkedList<ASemester> rv = new LinkedList<ASemester>();
...
//				- Preferences.prefs.coursesTaken.size()) {
	...
	// add a new child
	public ArrayList<ASemester> addChild() {

		ArrayList<ASemester> retVal = new ArrayList<ASemester>();
		...
		if (AScheduler.semesterBreakpoints.contains(Integer.valueOf(depth + 1))
			...
			for (int i = term + 1; i < Preferences.prefs.getNumSems(); i++) {
				for (Section candidate : AScheduler.directoryOfClasses.get(i)) {
		...
		for (Section candidate : AScheduler.directoryOfClasses.get(childTerm)) {
...
*/
			...
			*/
			...
			ASemester newChild = new ASemester(depth + 1, childTerm,
	...
	@Override
	public int compareTo(ASemester s) {
